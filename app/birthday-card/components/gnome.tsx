...
export function Gnome({ id, x, y, onClick, isVisible }: GnomeProps) {
  ...
  const handleClick = () => {
    setIsBouncing(true)
    onClick(x + 60, y + 80, id) // Center of gnome for fireworks, pass ID
  ...
  return (
    <button
      onClick={handleClick}
      className={`absolute cursor-pointer transition-all duration-500 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 rounded-full will-change-transform ${
